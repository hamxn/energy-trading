...
export class TransactionRBComponent {
  ...
  private bankCoinsRate = 1000;
  
  //define variables
  private cashExchanged;
  private coinsValue;
  ...
  execute(form: any): Promise<any> {
    ...
    if(this.action.value == 'getCash') {
        this.coinsValue = this.value.value;
    ...
    this.cashExchanged = this.bankCoinsRate * this.coinsValue;
  
    //create transaction object
    this.coinsToCashObj = {
      $class: "org.energy.trading.network.CoinsToCash",
      "coinsRate": this.bankCoinsRate,
    ...
    .then((result) => {
      ...
      if(result.value) {
        if ((result.value - this.cashExchanged) < 0 ){          
    ...
    .then((checkCash) => {
      ...
      {        
        ...
        .then((result) => {
          ...
          //check if enough value
          if(result.value) {
            if ((result.value - this.coinsValue) < 0 ){              
        ...
        .then((checkCoins) => {
          ...
          if(checkCoins)
          {           
            //call serviceTransaction call the cashToCoins transaction with coinsToCashObj as parameter 
            this.serviceTransaction.coinsToCash(this.coinsToCashObj)
            .toPromise()
            .then((result) => {
              this.errorMessage = null;
