...
describe('Starting tests for getorder', function() {
  ...
  it('website title should be getorder', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('getorder');
    })
  });

  it('network-name should be energy-trading@0.0.3',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('energy-trading@0.0.3.bna');
    });
  });

  it('navbar-brand should be getorder',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('getorder');
    });
  });
    ...
    it('EnergyToCoins component should be loadable',() => {
      page.navigateTo('/EnergyToCoins');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('EnergyToCoins');
      });
    });
  
    it('CoinsToCash component should be loadable',() => {
      page.navigateTo('/CoinsToCash');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('CoinsToCash');
      });
    });
